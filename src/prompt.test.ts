...
describe("buildCoachPrompt", () => {
  it("should contain the persona text in the output", () => {
    ...
    const result = buildCoachPrompt(personaText, DEFAULT_CONFIG.coachPrompt);
  ...
  it("should contain output JSON structure with all 4 categories", () => {
    const result = buildCoachPrompt("test persona", DEFAULT_CONFIG.coachPrompt);
  ...
  it("should not be vulnerable to template injection via {personaText} in input", () => {
    ...
    const result = buildCoachPrompt(maliciousText, DEFAULT_CONFIG.coachPrompt);
