...
C4Container
  ...
  Boundary(plugin, "Agent Persona Coach", "Plugin") {
    ...
    Container(injector, "CoachInjector", "TypeScript", "Formats nudges via formatNudge()")
  }
...
| `injector` | CoachInjector | Container | TypeScript | `injector.ts` — `formatNudge()` only (injectNudge removed with system.transform path) |
...
All containers run in the same Node.js process (this is a plugin, not a distributed system). The cache is in-memory only — no external database. The generator calls the LLM synchronously during `initializeSession()` and caches the result. The plugin now uses 3 hooks (was 4) — `system.transform` was removed, and initialization was moved to `chat.message` per [[adrs/0005-move-lazy-init-to-chat-message.adr.md]].
