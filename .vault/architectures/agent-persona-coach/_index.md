...
C4Context
  ...
  Rel(plugin, opencode, "Injects nudges via system-reminder blocks")
```
...
The plugin integrates with better-opencode via 4 hooks: `chat.message` (session init), `tool.execute.before` (rule compliance), `tool.execute.after` (cadence nudges), and `experimental.chat.system.transform` (system prompt injection). The LLM is only called once per unique agent persona (cached), making it a low-cost dependency.
